...
type Tab = "profil" | "sicherheit" | "sammlungen" | "tags";
...
export default function ProfilePage() {
  ...
  async function startMfaSetup() {
    ...
    setMfaLoading(false);
    if (res.ok) { const { qrDataUrl } = await res.json(); setMfaQr(qrDataUrl); setMfaToken(""); }
    else setMfaError(t.profile.mfaSetupError);
  }
  ...
  async function verifyMfa(e: FormEvent) {
    ...
    setMfaLoading(false);
    if (res.ok) {
      setMfaMsg(t.profile.mfaActivated); setMfaQr(null); setMfaToken("");
  ...
  async function disableMfa(e: FormEvent) {
    ...
    setMfaLoading(false);
    if (res.ok) {
      setMfaMsg(t.profile.mfaDeactivated); setShowDisable(false); setMfaDisableToken("");
  ...
  const tabs: { id: Tab; label: string }[] = [
    { id: "profil",    label: t.profile.tabProfil },
    { id: "sicherheit", label: t.profile.tabSicherheit },
    { id: "sammlungen", label: t.profile.tabSammlungen },
    { id: "tags",      label: t.profile.tabTags },
  ];
        ...
        <div className="max-w-xl space-y-8">
          <Section title={t.profile.appearance}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {([
                { id: "retro80s", label: t.profile.themeRetro80s, hint: t.profile.themeRetro80sHint, swatches: ["#0d0b1e", "#ff2d95", "#00f5d4", "#161429"] },
                { id: "retro90s", label: t.profile.themeRetro90s, hint: t.profile.themeRetro90sHint, swatches: ["#0e0e0e", "#e94560", "#f5a623", "#1a1a1a"] },
                { id: "modern",   label: t.profile.themeModern,   hint: t.profile.themeModernHint,   swatches: ["#0f172a", "#6366f1", "#06b6d4", "#1e293b"] },
              ] as const).map((th) => (
                <button key={th.id} onClick={() => applyColorTheme(th.id)}
                  ...
                  <div className="flex gap-1 mb-2">
                    {th.swatches.map((c, i) => <div key={i} className="h-4 flex-1 rounded" style={{ background: c }} />)}
                  </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <a href="/api/export?format=json" download className="rounded-md border border-border px-4 py-2 text-xs text-muted-foreground hover:border-primary hover:text-primary transition">{t.profile.exportJson}</a>
                  <a href="/api/export?format=csv" download className="rounded-md border border-border px-4 py-2 text-xs text-muted-foreground hover:border-primary hover:text-primary transition">{t.profile.exportCsv}</a>
                </div>
              </div>
          ...
          <Section title={t.profile.gdprTitle}>
            ...
            <a href="/api/export?format=gdpr" download className="inline-block rounded-md border border-border px-4 py-2 text-xs text-muted-foreground hover:border-primary hover:text-primary transition">
            ...
            {!profile?.mfaEnabled && mfaQr && (
              <div className="space-y-4">
                ...
                <form onSubmit={verifyMfa} className="flex gap-2">
                  <input type="text" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={mfaToken}
                    onChange={(e) => setMfaToken(e.target.value.replace(/\D/g, ""))} placeholder="123456"
                    className="retro-field w-32 font-mono text-center tracking-widest" required />
            ...
            {profile?.mfaEnabled && showDisable && (
              <form onSubmit={disableMfa} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <input type="text" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={mfaDisableToken}
                    onChange={(e) => setMfaDisableToken(e.target.value.replace(/\D/g, ""))} placeholder="123456"
                    className="retro-field w-32 font-mono text-center tracking-widest" required />
      ...
      {step === "confirm" && (
        <form onSubmit={handleDelete} className="space-y-3">
          ...
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            className="retro-field w-full" placeholder={t.profile.deleteAccountPasswordPlaceholder}
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { setStep("idle"); setPassword(""); setError(""); }} className="text-xs text-muted-foreground hover:text-foreground">
              {t.profile.mfaCancel}
            </button>
          </div>
        </form>
