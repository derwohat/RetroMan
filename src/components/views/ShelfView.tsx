      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="media-card group relative flex flex-col rounded-lg border border-border bg-card overflow-hidden">
            ...
            <Link href={`/collection/${categoryId}/${item.id}`} className="scanlines relative aspect-[3/4] bg-muted flex items-center justify-center overflow-hidden block">
              {imageUrl ? (
            ...
            <div className="p-2 flex flex-col flex-1">
              <div className="space-y-1">
                <Link href={`/collection/${categoryId}/${item.id}`} className="text-sm font-medium text-foreground line-clamp-2 leading-tight hover:text-primary transition-colors">{item.title}</Link>
