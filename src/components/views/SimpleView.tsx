      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="media-card group relative flex gap-3 rounded-lg border border-border bg-card p-3 overflow-hidden">
            {/* Thumbnail */}
            <Link href={`/collection/${categoryId}/${item.id}`} className="relative w-16 h-20 shrink-0 rounded-md bg-muted overflow-hidden flex items-center justify-center">
              {imageUrl ? (
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
              <div className="space-y-1">
                <Link href={`/collection/${categoryId}/${item.id}`} className="text-sm font-medium text-foreground line-clamp-2 leading-tight pr-14 hover:text-primary transition-colors">{item.title}</Link>
                ...
                {item.rating ? (
                  <div className="flex items-center gap-0.5">
                    {Array.from({ length: 5 }, (_, i) => (
                      <span key={i} className={`text-xs ${Math.round((item.rating ?? 0) / 2) > i ? "text-yellow-400" : "text-muted-foreground/30"}`}>★</span>
                    ))}
                  </div>
                ) : <span />}
